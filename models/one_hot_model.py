...
class OneHotSeq2Seq:
	...
	def __init__(self, rnn_type, latent_dim, num_encoder_tokens, num_decoder_tokens, target_token_index, target_index_token, max_decoder_seq_length, character_level):
	...
	def create_model(self, optimizer=None, line_length=300, summary=True):
		...
		elif self._rnn_type == "bi_gru":  # bi_gru
	...
	def _create_lstm(self):
		...
		# encoder
		encoder = keras.layers.LSTM(self._latent_dim, return_state=True, name="encoder")
		encoder_outputs, state_h, state_c = encoder(encoder_inputs)
		encoder_states = [state_h, state_c]
		# decoder input
		decoder_inputs = keras.Input(shape=(None, self._num_decoder_tokens), name="decoder_input")
		# decoder
		decoder = keras.layers.LSTM(self._latent_dim, return_sequences=True, return_state=True, name="decoder")
	...
	def _create_gru(self):
		...
		encoder_inputs = keras.Input(shape=(None, self._num_encoder_tokens), name="encoder_input")
		# encoder
		encoder = keras.layers.GRU(self._latent_dim, return_state=True, name="encoder")
		encoder_outputs, state = encoder(encoder_inputs)
		encoder_states = [state]
		# decoder input
		decoder_inputs = keras.Input(shape=(None, self._num_decoder_tokens), name="decoder_input")
		# decoder
		decoder = keras.layers.GRU(self._latent_dim, return_sequences=True, return_state=True, name="decoder")
	...
	def _create_bidirectional_lstm(self):
		...
		encoder_inputs = keras.Input(shape=(None, self._num_encoder_tokens), name="encoder_input")
		# encoder
		encoder = keras.layers.Bidirectional(keras.layers.LSTM(self._latent_dim, return_state=True), name="encoder")
		...
		decoder_inputs = keras.Input(shape=(None, self._num_decoder_tokens), name="decoder_input")
		# decoder
		decoder = keras.layers.LSTM(self._latent_dim * 2, return_sequences=True, return_state=True, name="decoder")
		decoder_outputs, _, _ = decoder(decoder_inputs, initial_state=encoder_states)
		# decoder dense output
		decoder_dense = keras.layers.Dense(self._num_decoder_tokens, activation='softmax', name="decoder_dense")
		dense_outputs = decoder_dense(decoder_outputs)
	...
	def _create_bidirectional_gru(self):
		...
		encoder_inputs = keras.Input(shape=(None, self._num_encoder_tokens), name="encoder_input")
		# encoder
		encoder = keras.layers.Bidirectional(keras.layers.GRU(self._latent_dim, return_state=True), name="encoder")
		...
		decoder_inputs = keras.Input(shape=(None, self._num_decoder_tokens), name="decoder_input")
		# decoder
		decoder = keras.layers.GRU(self._latent_dim * 2, return_sequences=True, return_state=True, name="decoder")
		decoder_outputs, _ = decoder(decoder_inputs, initial_state=encoder_states)
		# decoder dense output
		decoder_dense = keras.layers.Dense(self._num_decoder_tokens, activation='softmax', name="decoder_dense")
		dense_outputs = decoder_dense(decoder_outputs)
