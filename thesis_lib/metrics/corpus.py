...
def corpus_bleu(references: List[str], predictions: List[str], smoothing_function=bleu_score.SmoothingFunction().method1):
	if len(references) != len(predictions):
		raise ValueError("The lists must have the same length")

	references = [[o] for o in references]

	return bleu_score.corpus_bleu(references, predictions, smoothing_function=smoothing_function)
